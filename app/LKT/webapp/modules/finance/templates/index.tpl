...
<div class="pd-20">
	<div class="text-c">
		<form action="index.php?module=finance" method="get">
            ...
            <tbody>
	            {foreach from=$list item=item name=f1}
	                <tr class="text-c">
	                    ...
	                    <td>
	                    	{if $item->status == 0}
	                        <a style="text-decoration:none" class="ml-5" href="index.php?module=finance&action=del&m=1&id={$item->id}&name={$item->name}&money={$item->money}&s_charge={$item->s_charge}" title="审核通过" onclick="return confirm('确定要通过该用户的申请?')"><i class="Hui-iconfont">&#xe6a7;</i></a>
	                        <a style="text-decoration:none" class="ml-5" href="index.php?module=finance&action=del&m=2&id={$item->id}&name={$item->name}&money={$item->money}&s_charge={$item->s_charge}" title="审核不通过" onclick="return confirm('确定要拒绝该用户的申请?')"><i class="Hui-iconfont">&#xe6a6;</i></a>
	                        {/if}
	                    </td>
	                </tr>
	            {/foreach}
