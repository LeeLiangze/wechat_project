...
class list1Action extends Action {
    ...
    public function getDefaultView() {
        ...
        $sql = "select user_id from lkt_withdraw where status = 2 ";
        $r = $db->select($sql);
        $total = 0;
        ...
        if($r){
            ...
            if($pageto == 'all') { // 导出全部
                ...
                $db->admin_record($admin_id,' 导出提现拒绝列表第 '.$page.'页'.$pagesize.'条数据',4);
            }else{ // 不导出
        ...
        $url = "index.php?module=finance&action=list1&name=".urlencode($name)."&Bank_card_number=".urlencode($Bank_card_number)."&Cardholder=".urlencode($Cardholder)."&pagesize=".urlencode($pagesize);
