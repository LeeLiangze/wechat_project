...
<section class="rt_wrap content mCustomScrollbar" style="margin-left:20px;margin-right:10px;height:100%;position:relative;">
 <div class="rt_content" style="font-size: 40px;">
      <div class="page_title" style="text-align:center;border: 0;">
       <h2 class="fl" >订单详情</h2>
      </div>
      <table class="table">
       <tr>
        ...
        <td>订单类型：{if $data.otype=='pt'}<span >拼团订单</span>{else}{if $data.drawid > 0}<span >抽奖订单</span>{else}<span>普通订单</span>{/if}{/if}</td>
       </tr>
       <tr>
        <td style="width: 160px;">下单时间：{$data.add_time}</td>
        <td>发货时间：{$data.deliver_time}</td>
        <td>到货时间：{$data.arrive_time}</td>
        <td>订单号：{$data.sNo}</td>
       </tr>
       <tr>
        <td style="width: 160px;">订单状态：<a>{$data.r_status}</a></td>
        <td >退货原由：<mark>{$data.content}</mark></td>
      ...
      <table class="table">
       ...
       <tr>
        ...
        <td class="center" id="p_name">{$item->p_name}</td>
        <td class="center">{$item->size}</td>
        <td class="center">{$item->p_id}</td>
        <td class="center"><span style="color:#19a97b;font-weight: bold;">￥{$item->p_price}</span></td>
        <td class="center"><strong>{$item->num}</strong></td>
        <td class="center"><span style="color:#19a97b;font-weight: bold;">￥{$item->z_price}</span></td>
      ...
      <DIV  id="dayinDiv" name="dayinDiv">
       ...
       {else}
       {if $data.gstatus==1}
       <input type="button" value="发货" class="group_btn ptfh" onclick="system_category_add('发货','index.php?module=orderslist&action=addsign&id={$data.sNo}&otype=pt',400,300)" />
       {elseif $data.gstatus==10}
       <input type="button" value="退款" class="group_btn" onclick="system_category_del(this,{$data.id},{$data.z_price})" />
       {/if}
       {/if}
...
function system_category_del(obj,id,control){
    ...
    $.ajax({
      ...
      dataType: 'json',
      data:{id:id,price:control,sNo:sNo,p_name:p_name,paytype:paytype,trade_no:trade_no,uid:uid},
      success: function(data){
        if(data.status == 1){
