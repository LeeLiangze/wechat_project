...
// $status =0;
      ...
      $sql1 = 'select id,sNo,name,sheng,shi,xian,address,mobile,z_price,status,drawid from lkt_order order by id asc';
      ...
      if($sNo !== false && $status == false){
        $condition .= ' and (`sNo` like "%'.$sNo.'%"  or `name` like "%'.$sNo.'%" or `mobile` like "%'.$sNo.'%") order by id asc';
        $sql1 .= $condition;
       }

      if($status !== false && $sNo !== false){
        $sql1 = 'select id,sNo,name,sheng,shi,xian,address,mobile,z_price,status from lkt_order where status="'.$status.'" and (`sNo` like "%'.$sNo.'" or `name` like "%'.$sNo.'%" or `mobile` like "%'.$sNo.'%") order by id asc';
      }
