...
class examineAction extends Action {
	...
	public function getDefaultView() {
		...
		$sNo = $res_p[0] -> r_sNo;
		//判断单个商品退款是否有使用优惠
		$sql_id = "select a.id,a.trade_no,a.sNo,a.pay,a.z_price,a.user_id,a.allow,a.spz_price,a.reduce_price,a.coupon_price,m.p_price from lkt_order as a LEFT JOIN lkt_order_details AS m ON a.sNo = m.r_sNo where m.id = '$id' and a.status = '4' ";
					...
					if ($pay == 'wallet_Pay') {
						...
						if(empty($price)){
							...
							if ($res_d == $res_o) {
								...
								$price = number_format($order_res[0] -> z_price / $spz_price * $p_price, 2);
								// var_dump($spz_price,$order_res[0]->z_price,$p_price,$price);exit;
							}							
						}
						...
						if ($refund_wx > 0 && !empty($refund_wx)) {
							...
							$rr = $db -> insert($sqll);
							if ($wxtk_res['return_msg'] == 'OK') {
								$wxres_t = $wxtk_res['return_msg'];
							}
						}
