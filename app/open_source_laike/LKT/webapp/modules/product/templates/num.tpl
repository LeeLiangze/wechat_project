            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c">
                    <td>{$smarty.foreach.f1.iteration}</td>
                     <td>{if $item->imgurl != ''}<img onclick="pimg(this)" style="width: 50px;height: 50px;" src="{$uploadImg}{$item->imgurl}">{else}<span>暂无图片</span>{/if}</td>
                    <td>
                        <p>{$item->product_title}</p>
                        {foreach from=$item->res item=item1 name=f1}
                            <text>{$item1}</text>
                        {/foreach}
                    </td>
                    <td>{$item->add_date}</td>
                    ...
                    <td>
                        ...
                        {/if}
                        {*<a style="text-decoration:none" class="ml-5" title="下架" onclick="product_lower({$item->sx_id})"><i class="Hui-iconfont">&#xe6de;</i></a>*}

                        {*<a style="text-decoration:none" class="ml-5" title="上架" onclick="product_up({$item->sx_id})"><i class="Hui-iconfont">&#xe6dc;</i></a>*}

                        <a style="text-decoration:none" class="ml-5" href="index.php?module=product&action=del&id={$item->id}" onclick="return confirm('确定要删除此产品吗?')"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    </td>
                </tr>
            {/foreach}
            </tbody>
